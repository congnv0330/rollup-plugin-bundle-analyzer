...
const SIZE_SWITCH_ITEMS = [
  { label: 'entry', prop: 'statSize' },
  { label: 'render', prop: 'parsedSize' },
  { label: 'gzipped', prop: 'gzipSize' }
];

const viewType = [
  { label: 'chunk', prop: 'default' },
  { label: 'library', prop: 'lib' }
];
...
export default class ModulesTreemap extends Component {
        ...
        >
          <div style={{ marginBottom: '10px' }}>
            <Switcher
              label="view"
              items={viewType}
              activeItem={this.activeViewType}
              onSwitch={this.handleViewTypeSwitch}
            />
          </div>
          <div className={s.sidebarGroup}>
            ...
            {store.hasConcatenatedModules && (
              <div className={s.showOption}>
                <Checkbox checked={viewType} onChange={this.handleConcatenatedModulesContentToggle}>
  ...
  @computed get activeViewType() {
    return viewType.find((item) => item.prop === store.viewType);
  }
  ...
  handleViewTypeSwitch(type) {
    store.viewType = type.prop;
  }
